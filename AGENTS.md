...
- `/home/nicholas/Dokumenter/git/LumenShell/lumen-desktop/src/window.vala` — `DesktopWindow`. Layer-shell namespace `lumen-desktop`, layer `BACKGROUND`, all four anchors, `exclusive_zone = 0` (does NOT consume zone — distinct from Kickoff's `-1`), `KeyboardMode.ON_DEMAND` (keyboard follows pointer: clicking the search entry grants us keys, clicking another window hands them back). Inline `DESKTOP_CSS` mirrors `KICKOFF_CSS` but `.lumen-desktop-root` has transparent background (compositor wallpaper shows through) and the search class is renamed `.desktop-search`. Key controller installed without CAPTURE phase. Same Alt+1/2/3, Ctrl+Backspace, Left/Right shortcuts as Kickoff but **no ESC handler**. `AppEntry.launched` is wired to `reset_view()` (clear query + return to page 0) instead of a hide; the desktop never hides itself. `notify["is-active"]` → `PeekIpc.start()` so gaining focus triggers the [[wayfire-desktop-peek]] plugin. The full body (search row + grid + dots) is wrapped in a `BlurBin`; `DesktopToplevels.focus_changed` drives a tick-callback fade between 0 and `BLUR_MAX = 18` so the desktop blurs while you work and clears the moment you click through.
...
- `/home/nicholas/Dokumenter/git/LumenShell/lumen-desktop/src/widgets/blur_bin.vala` — single-child `Gtk.Widget` that wraps `Gtk.Snapshot.push_blur(radius)` around its child's snapshot. GPU-native GSK blur — no offscreen Cairo. Radius below 0.5 short-circuits to a plain `snapshot_child` so idle frames pay nothing. Owned by `DesktopWindow`; the window animates `radius` via a tick callback (ease-out-cubic, 800 ms in / 300 ms out, target `BLUR_MAX = 18`).
- `/home/nicholas/Dokumenter/git/LumenShell/lumen-desktop/src/wl/toplevels.vala` — minimal foreign-toplevel watcher (`DesktopToplevels`). Same wlhooks bind pattern as lumen-panel's `ToplevelStore` (`init_toplevel_with_display` on GTK's wl_display, `register_on_window_{new,rm,focus}`) but exposes only one bit of state: `any_focused`, with a `focus_changed(bool)` signal. That signal drives the blur fade — fade in when a real toplevel takes focus, fade out otherwise (and a wallpaper click forces a short-duration fade-out on top of that).
