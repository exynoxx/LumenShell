...
public class App : GLib.Object {
    ...
    public void activate (Gtk.Application app) {
        ...
        // The system tray is a singleton (the SNI watcher owns a single DBus
        // name); it is built once and lives on the primary panel.
        tray = new TrayBar();
        ...
        tray.add_paged(new ExitTray(logind_service.bridge));
    ...
    void build_windows () {
        ...
        for (uint i = 0; i < n; i++) {
            ...
            bool host = (i == 0);
            windows.add(new PanelWindow(app, mon, host, host ? tray : null));
        }
    }
