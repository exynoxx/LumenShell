...
static void seat_capabilities(void *data, struct wl_seat *s, uint32_t capabilities) {
    ...
    if ((capabilities & WL_SEAT_CAPABILITY_KEYBOARD) && grab_keyboard && !keyboard) {
...
static void seat_registry_handler(void *data, struct wl_registry *registry,
                                  ...
                                  uint32_t version) {
    // Take only the first advertised seat. Multi-seat configurations are rare
    // for taskbars; revisit if a use case appears.
    if (seat) return;
...
void seat_init(void) {
    ...
    if (!xkb_context) {
        fprintf(stderr, "seat: xkb_context_new failed\n");
    }
...
void seat_cleanup(void) {
    if (xkb_state)   { xkb_state_unref(xkb_state);     xkb_state   = NULL; }
    if (xkb_keymap)  { xkb_keymap_unref(xkb_keymap);   xkb_keymap  = NULL; }
    if (xkb_context) { xkb_context_unref(xkb_context); xkb_context = NULL; }

    if (keyboard) { wl_keyboard_release(keyboard); keyboard = NULL; }
    if (pointer)  { wl_pointer_release(pointer);   pointer  = NULL; }
    if (seat)     { wl_seat_release(seat);         seat     = NULL; }
}
